    ...
    return function(obj, only_added, callback) {
      if (MutationObserver) {
        ...
        var obs = new MutationObserver(function(mutations, observer) {
          if (only_added) {
            if (mutations[0].addedNodes.length) callback();
          } else {
    ...
    if (containers.length > 0) {
      for (var i = 0; i < containers.length; i++) {
        ...
        gallery.infiniteScroll({
          ...
          append:
            '.rl-gallery-container[data-gallery_id="' +
            gallery_id +
            '"] .rl-gallery-item' +
            (gallery.hasClass("rl-masonry-gallery") || gallery.hasClass("rl-basicmasonry-gallery") ? "-no-append" : ""),
          status: false,
      ...
      if (args.woocommerce_gallery === "1") {
        ...
        if (gallery.find(".woocommerce-product-gallery__trigger").length === 0) {
          ...
          gallery.on("click", ".woocommerce-product-gallery__trigger", function(e) {
            ...
            if (flex.length) flex.find(".flex-active-slide a[data-rel]").trigger("click");
            else
              gallery
                .find("a[data-rel]")
                .first()
    ...
    switch (script) {
      ...
      case "prettyphoto":
        ...
        $('a[rel*="' + selector + '"], a[data-rel*="' + selector + '"]').prettyPhoto({
          ...
          social_tools:
            args.social === "1"
              ? '<div class="pp_social"><div class="twitter"><a href="//twitter.com/share" class="twitter-share-button" data-count="none">Tweet</a><script type="text/javascript" src="//platform.twitter.com/widgets.js"></script></div><div class="facebook"><iframe src="//www.facebook.com/plugins/like.php?locale=en_US&href=' +
                location.href +
      ...
      case "imagelightbox":
        ...
        $('a[rel*="' + selector + '"], a[data-rel*="' + selector + '"]').each(function(i, item) {
          ...
          // check data-rel attribute first
          if (typeof attr !== "undefined" && attr !== false && attr !== "norl") selectors.push(attr);
          // if not found then try to check rel attribute for backward compatibility
          else {
            attr = $(item).attr("rel");
        ...
        if (selectors.length > 0) {
          ...
          $(selectors).each(function(i, item) {
            ...
            tos.bind("sliding.tos", function(event, number) {
              last_image = $($(event.target).find(".tos-slider .tos-slide")[number])
                .find("img")
                .attr("src");
